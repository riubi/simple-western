...
mixin PositionObstaclable on PositionComponent implements PositionInhiber {
  ...
  Offset inhib(PositionComponent movable, Offset positionDelta) {
    ...
    if (positionDelta.dy != 0 && !_isMovementBlocked(position.y, movable.position.y, size.y, movable.size.y, positionDelta.dy)) {
      positionDelta = Offset(positionDelta.dx, 0);
    }

    if (positionDelta.dx != 0 && !_isMovementBlocked(position.x, movable.position.x, size.x, movable.size.x, positionDelta.dx)) {
